...
class speech_control():
    def __init__(self):
        self.musicPath = '/home/fjj/documents/fjj_ros_code/fjj_test_code/src/homework/sounds/youbelongwithme.mp3'
        self.question_start_signal = "/home/fjj/documents/fjj_ros_code/fjj_test_code/src/homework/sounds/question_start_signal.wav"
    ...
    def baiduCallback(self, msg):
        ...
        if msg.data.strip()=='':
            ...
            if "failed" in output:
                ...
                if ( "hahaha" in output or "haha" in output or "wow" in output or "ha" in output or "you" in output or "dog" in output or "photo" in output or "go" in output or "out" in output or "back" in output):
                    if "want" in output or "leave" in output or "to" in output or "i'll" in output or string == "i want to go":
                        self.sh.say("i heard that you want to go, all right, i'm ready", self.voice)
                        self.tur_control_pub.publish("go")
                        self.is_answer_question = False
                    elif "hahaha" in output or "haha" in output or "wow" in output or "ha" in output or "out" in output or "let's" in output or "dog" in output or "door" in output:
                        self.sh.say("ok let's go to the taxi", self.voice)
                        self.tur_control_pub.publish("out")
                        self.is_answer_question = False
                    elif "back" in output or "you" in output:
