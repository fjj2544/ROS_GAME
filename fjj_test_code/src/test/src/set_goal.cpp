...
protected: 
    ...
    geometry_msgs::Pose door_pos;//我就是应该恢复到door_pose那个地方,本来就没有变化
...
fare_well::fare_well():mbc(nh,"move_base",true)//记住应该旋转,不能不动..不动就容易搞不到位置.
    {
        ...
        /*-----------------------------------------------------------------------------------------------set_pose-------------------------------------------------------------*/
        //这里认为去门就是去停车点，开始观察。
        set_pos(door_pos,-5.08,-1.47,0.0102,0.0,0.0,-0.484099,0.875);//设置门的位置  甚至我可以去那个cab 我是不是可以去停车点，然后找到准确的车子,也就是一个识别的过程
        ...
        out_door_pose.pose.pose = door_pos;
        /*-----------------------------------------------------------------------------------------------pub_msg-------------------------------------------------------------*/
...
/*--------------------------------------------------------------------------------找到正确的司机信息,并没有说没有外面的地图吧。本来就不应该用follow me ,因为本来就是我guide 别人----------------------------------------------------------*/
...
void fare_well::cb(const std_msgs::String::ConstPtr & msg )
    {
      ...
      std::string voice_command;
       ...
       /*-----------------------------------------------------------------------------------------------语音开启机器人运动-------------------------------------------------------------*/
       //这个时候你可以调整位姿,语音最好作为控制中枢  最后必须确定这个人想走，也就是识别之后可以再问一下
