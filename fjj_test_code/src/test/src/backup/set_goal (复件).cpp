...
//比如launch中启动节点到底是CPP文件名字,还是cmakelist中的名字还是什么? 果然是cmakelist中的节点名称,那么这里就无所谓了..
...
//格式化字符串
void delete_str(std::string & str)
{
    
    string temp = "。";
    str.erase(str.find(temp));
}
//得到当前点的坐标
...
fare_well::fare_well():mbc(nh,"move_base",true)//记住应该旋转,不能不动..不动就容易搞不到位置.
    {
        ...
        set_pos(shelf_pos,-5.1271,1.865,0.0102,0.0,0.0,0.82319,0.5677);//设置储物柜位置
...
void fare_well::cb(const std_msgs::String::ConstPtr & msg )
    {
       ...
       if(start_call=="i want to go。")//你可以走了
       {
        robot_go_to_shelf=true;
       }
       if(start_call=="you can go back。"||start_call=="yes you can。")
       {
           robot_go_back=true;
       }
       if(start_call=="take me out。")
       {
